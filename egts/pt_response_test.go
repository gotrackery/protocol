...
func TestEgtsPktResp_Encode(t *testing.T) {

	posDataBytes, err := egtsPktResp.Encode()
	if assert.NoError(t, err) {
		assert.Equal(t, posDataBytes, testEgtsPkgBytes)
	}
}

func TestEgtsPktResp_Decode(t *testing.T) {
	egtsPkg := Packet{}

	err := egtsPkg.Decode(testEgtsPkgBytes)
	if assert.NoError(t, err) {
		assert.Equal(t, egtsPkg, egtsPktResp)
	}
}
